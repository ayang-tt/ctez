...
let ctez_target ((target, minted): ctez_target) (storage : storage) =
    if Tezos.get_sender () <> storage.ctez_address then
        ...
        let updated_target = (target, minted) in
   ...
   let { to_ = to_ ;
    ...
    if Tezos.get_now () >= deadline then
        ...
        let cash_bought =
            ...
            let (target, _) = storage.target in
            let bought = trade_dtez_for_dcash tezPool storage.cashPool tezSold target rounds in
...
let cash_to_tez (param : cash_to_tez) (storage : storage) =
    ...
    if (Tezos.get_now ()) >= deadline then
        ...
        let tez_bought =
            let (target, _) = storage.target in
            let bought = trade_dcash_for_dtez storage.tezPool storage.cashPool cashSold target rounds in
            let bought_after_fee = bought * const_fee / const_fee_denom in
        ...
        let op_cash = cash_transfer storage sender self_address cashSold in
...
(* Entrypoint to allow depositing tez. *)
    ...
    let storage = {storage with tezPool = storage.tezPool + mutez_to_natural amount} in (([] : operation list), storage)
...
let tez_to_token (param : tez_to_token) (storage : storage) : result =
    ...
    if Tezos.get_now () >= deadline then
      (failwith error_THE_CURRENT_TIME_MUST_BE_LESS_THAN_THE_DEADLINE : result)
        ...
        let amount = Tezos.get_amount () in
        let tezSold = mutez_to_natural amount in
        (* We don't check that cashPool > 0, because that is impossible unless all liquidity has been removed. *)
        let (target, _) = storage.target in
        let cash_bought =
           (let bought = trade_dtez_for_dcash storage.tezPool storage.cashPool tezSold target rounds in
...
let update_consumer (operations, storage : result) : result =
    if storage.lastOracleUpdate = Tezos.get_now ()
        ...
        | Some c -> c in
        let (target, _) = storage.target in
        ((Tezos.transaction ((marginal_price storage.tezPool storage.cashPool target), storage.cashPool) 0mutez consumer) :: operations,
        {storage with lastOracleUpdate = Tezos.get_now ()})

let get_marginal_price (param : get_marginal_price) (storage : storage) : result =
    let (target, _) = storage.target in
    let price = marginal_price storage.tezPool storage.cashPool target in
